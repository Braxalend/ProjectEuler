...
def MulInLine(arr):
    ...
    for i in range(a.shape[0]):
        for j in range(a.shape[1] - (p-1)):
            r += [functools.reduce(lambda x, y: x * y, [int(k) for k in arr[i][j:j+p]])]
    return numpy.array(SplitArr(r,a.shape[1] - (p-1)))

def MulInRDiag(arr):
    p = 4
    r = []
    for i in range(a.shape[0] - (p - 1)):
        for j in range(a.shape[1] - (p - 1)):
            ta = []
            for k in range(p):
                ta += [arr[i+k][j+k]]
            r += [functools.reduce(lambda x, y: x * y, ta)]
    return numpy.array(SplitArr(r, a.shape[1] - (p - 1)))
...
a = numpy.array(SplitArr(l,20))
b = numpy.transpose(a)
c = numpy.flipud(a)

res = [max(MaxInLine(MulInLine(a))),
       max(MaxInLine(MulInLine(b))),
       max(MaxInLine(MulInRDiag(a))),
       max(MaxInLine(MulInRDiag(c)))]
print(max(res))
